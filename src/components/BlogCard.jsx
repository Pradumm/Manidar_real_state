...
const BlogCard = ({ blog }) => {
    ...
    function truncateHtml(html, limit) {
        if (!html) return ""; // Check if html is defined
        ...
        // If the number of words exceeds the limit, truncate
        if (words.length > limit) {
          words = words.slice(0, limit);
          return `${words.join(" ")}...`;
        }
      
        // If the number of words is within the limit, return the original HTML
        return html;
      }
        ...
        <>
            ...
            <div className='lg:max-w-sm relative p-1 bg-white shadow-lg  overflow-hidden rounded-lg border-[1px] border-[#f1f8ff] text-[#464646] '>
                ...
                <img
                    ...
                    src={`https://backend.artechworld.tech/uploads/blogs/tImages/${blog?.mainImage}`} alt='' />
                <div className='p-4'>
                    <h4 className='text-xl cursor-pointer hover:text-NewYello font-semibold py-1'>{blog?.title}</h4>
                    <p>{new Date(blog?.created_at).toLocaleDateString()}</p>
                    {/* Render HTML content using dangerouslySetInnerHTML */}
                    <p  className='mb-6' dangerouslySetInnerHTML={{ __html: truncateHtml(blog?.content, 30) }}></p>
